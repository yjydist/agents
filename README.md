...
本仓库提供可直接复制的 [工程原则模板](template/agent.md) 与 [技能](skills), 以敏捷开发和持续设计为主线, 帮助 Agent 在交付, 验证和反馈中满足真实需求, 持续降低软件的理解与修改成本. 模板存放在 `template/agent.md`, 避免被当作本仓库的 Agent 指令加载.
...
技能的 `description` 用于描述能力与适用场景, 正文提供工作方法. 具体发现方式由目标工具决定; 描述清晰不等于保证所有模型都能自动选择正确技能.
